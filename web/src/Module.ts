...
import {MovingController} from './game/engine/MovingController';
...
setCached(MovingController, () => new MovingController());
...
setCached(Keyboard, () => new Keyboard(get(MovingController), get(Game)));
setCached(GamePad, () => new GamePad(get(MovingController), get(Game)));
setCached(Spells, () => new Spells());
setCached(World, () => new World(get('map')));
setCached(Game, () => new Game(get('api'), get(World), get(MovingController), get(Spells), get(Audios)));
