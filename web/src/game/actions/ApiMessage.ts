...
export type ActorMoved = { mv: Dir | null, sight: Dir, x: pos, y: pos, speed: speed, offset: uint, creatureId: uid }
...
export type CreatureHid = { creatureId: uid }
...
export const API_MAPPER: ApiMapper = {
    ...
    'actor_hid': (m: any): CreatureHid => (
        {creatureId: m.creatureId}
    ),
    'damage': (m: any): Damage => (
        {id: m.id, creatureId: m.creatureId, victimId: m.victimId, amount: m.amount, spellId: m.spellId, crit: m.crit}
    ),
    'death': (m: any): Death => (
        {id: m.id, creatureId: m.creatureId, victimId: m.victimId}
    ),
    ...
    'actor_moved': (m: any): ActorMoved => (
        {creatureId: m.creatureId, x: m.x, y: m.y, offset: m.offset, speed: m.speed, mv: m.mv, sight: m.sight}
    ),
}